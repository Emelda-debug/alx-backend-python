...
def element_length(lst: List[Any]) -> List[Tuple[Any, int]]:
    ...
    we know that result is a list, where our iterator will
    loop over the elements in our list
    Furthermore, we know that this will return any value 
    for i because i can be anything eg. number,
    string etc  and int for len(i) be cause lenght will
    always return a number int. We also know that this list
